...
def analyze_news_items(articles: list[NewsArticle]) -> list[AnalyzedNews]:
    return sorted([analyze_news_item(article) for article in articles], key=lambda item: item.impact_score, reverse=True)


def preview_news_items(articles: list[NewsArticle]) -> list[AnalyzedNews]:
...
def _default_category(category: str) -> str:
    return {"finance": "市场", "politics": "政策", "culture": "消费", "world_bank": "长期发展"}.get(category, "市场")
