...
from src.ui.market_components import render_market_loading_status
...
def render_top_banner() -> None:
    ...
    with action_cols[0]:
        update_market_clicked = st.button("更新指数", use_container_width=True, type="primary")
    with action_cols[1]:
        if st.button("刷新数据", use_container_width=True, type="primary"):
            st.cache_data.clear()
            st.session_state.pop("market_payload", None)
            st.session_state.pop("market_update_feedback", None)
            st.rerun()
    with action_cols[2]:
        if update_market_clicked:
            with st.spinner("指数更新中..."):
                get_market_payload.clear()
                st.session_state["market_payload"] = get_market_payload()
            st.session_state["market_update_feedback"] = st.session_state["market_payload"]
        render_market_loading_status(st.session_state.get("market_update_feedback"))
    st.markdown(
        """
...
if page == "Settings":
    ...
    market_payload = st.session_state.get("market_payload") or get_market_payload()
    debug_market_payload("home", market_payload)
    market_data = market_payload.get("data") or {"indices": [], "snapshot": None}
    render_market_dashboard(
        market_data.get("indices", []),
        analyzed_news,
        world_bank_documents,
        get_world_bank_indicators("CHN"),
        market_data.get("snapshot"),
        [],
        False,
    ...
    akshare_snapshot = market_data.get("snapshot")
    render_market_dashboard(
        market_indices,
        analyzed_news,
        world_bank_documents,
    ...
    analyzed_news = filter_by_selected_tags(analyzed_news, selected_tags)
    market_payload = st.session_state.get("market_payload") or get_market_payload()
    market_indices = (market_payload.get("data") or {}).get("indices", [])
